...
class Trainer(LinearHeadTrainer):
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        # ================== R2T 自适应裁剪设置 ==================
        # 定义几何增长的候选 C 值列表（可根据需要调整）
        C_candidates = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]
        num_candidates = len(C_candidates)
        # 为每个候选分配隐私预算（并行组合）
        per_candidate_epsilon = self.args.dp_epsilon / num_candidates
        # 预计算每个候选的噪声标准差（基于各自的 epsilon 和对应的 C）
        ...
        if self.args.dpzero:
            ...
            for C in C_candidates:
                ...
                # 注意：噪声标准差公式来自 DPZero 原始代码：2 * multiplier * C / total_train_batch_size
                ...
                if self.args.zero_order_optim:
                    ...
                    for _ in range(num_zs):
                        ...
                        if self.args.dpzero:
                            ...
                            # 非 DPZero 模式，直接使用原始梯度（不裁剪不加噪）
                            projected_grad = projected_grad_raw
                        # ====================================================

                        # 后续处理（梯度累积、参数更新等）保持不变
