...
class Trainer(LinearHeadTrainer):
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if transformers.is_torch_tpu_available():
            ...
            # 修正：使用 self.args.local_rank 而不是 args.local_rank
            world_size = (
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.zero_order_optim:
                    ...
                    for _ in range(num_zs):
                        ...
                        # ========== 新增：记录裁剪和加噪声之前的原始梯度值 ==========
                        # 注意：projected_grad 是一个标量张量，取 .item() 转为 Python 数值
                        # 记录原始梯度的均值（一个标量）
                        self.raw_gradients.append(projected_grad.mean().item())
                        # ========================================================
                        # 在记录梯度的位置（约第 474 行）添加：
                        if self.state.global_step == 0:  # 只在第一步打印一次，避免刷屏
